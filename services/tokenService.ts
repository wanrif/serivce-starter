...
export const createTokenService = async (tokenData: Partial<IToken>): Promise<IToken> => {
  const token = new Token(tokenData);
  return await token.save();
};

export const updateTokenService = async (key: string, tokenData: Partial<IToken>): Promise<IToken | null> => {
...
export const deleteTokenService = async (key: string): Promise<IToken | null> => {
  const query: any = {
    $or: [{ access_token: key }],
  };
...
export const createTwoFATokenService = async (tokenData: Partial<IToken>): Promise<IToken> => {
